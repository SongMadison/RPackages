# The code is revised from:
# http://datascience.vegas/ggplot2-elegant-graphics-data-analysis/#sthash.efSF0IBw.vR4bSeDm.dpbs
...
DF <- data.frame(x = seq(40), y = rnorm(40))
...
# Add a line of text
p6 <- p5 + annotate("text", x = 0, y = 2, hjust=0, vjust =1, label = "Some text")
p5 + geom_text(x = 0, y = 2, hjust=0, vjust =1, label = "Some text")
...
ggplot(DF1, aes(x = x)) +
    geom_histogram(aes(y = ..count.. * 0.2),
...
# Let's party!
p + geom_text(data = formdat, aes(label = txt, x=cty, y =hwy ), hjust = 0, size = 6,
              parse = TRUE) +
    facet_wrap(~ cyl)
